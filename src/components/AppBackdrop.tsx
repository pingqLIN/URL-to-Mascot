...
function AppBackdrop({ bgDepthMap, bgOverride, bgSequence, previewMode = false }: AppBackdropProps) {
  ...
  useEffect(() => {
    ...
    const applyFocusLighting = () => {
      ...
      const farFactor = normalizedDepth;
      ...
      const lightCoreAlpha = 0.014 + nearFactor * 0.096;
      const lightGlowAlpha = 0.003 + nearFactor * 0.028;
  ...
  return (
    <div ref={rootRef} className="pointer-events-none fixed inset-0 overflow-hidden">
      ...
      <div
        ...
        className="hero-gold-dust-field absolute left-[37.5%] top-0 bottom-[clamp(150px,22vh,280px)] w-[25%] overflow-hidden"
      >
        {dustElements}
      </div>
      <AnimatePresence>
