...
function buildConceptSignature(result: ConceptResult) {
  return [
...
function ConceptSection({
  ...
  const conceptDirty = Boolean(result) && buildConceptSignature(result) !== conceptPromptSyncSignature;
  ...
  useEffect(() => {
    ...
    setConceptPromptSyncSignature(buildConceptSignature(result));
  }, [hasResult, result?.section6.content, url]);
